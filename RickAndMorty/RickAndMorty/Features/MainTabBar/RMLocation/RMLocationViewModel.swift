...
final class RMLocationViewModel: NavigationBarViewModel, RMLocationViewModelProtocol {
    ...
    func fetchLocation(with url: String?) {
        ...
        animationHandler.animate { [weak self] in
            ...
            self?.requestManager.request(url: url, httpMethod: .get, object: RMLocationEndpointJson.self) { result in
                switch result {
    ...
    enum RMDetailCell: Int, CaseIterable {
        ...
        func presentDetail(controller: UIViewController?, location: RMMainLocation) {
            let cellModel: RMCellInfoDisplayViewModel.RMCellModel = RMCellInfoDisplayViewModel.RMCellModel(title: getTitle, value: getValue(for: location))
...
extension RMLocationViewModel: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if (indexPath.row == 4) && !(locationResult[indexPath.section].residents.isEmpty) {
            ...
            for url in charUrls {
                group.enter()
                requestManager.request(url: url, httpMethod: .get, object: RMCharacterResultsJson.self) { result in
                    defer { group.leave() }
                    switch result {
            ...
            group.notify(queue: .global()) { [weak self] in
                ...
                for n in 0..<chars.count {
                    group2.enter()
                    self?.requestManager.request(url: chars[n].image, httpMethod: .get) { result in
                        switch result {
                ...
                group2.notify(queue: .main) {
                    ...
                    let viewModel: RMCharacterCollectionFromLocationViewModel = RMCharacterCollectionFromLocationViewModel(characters: chars)
                    let view: RMCharacterCollectionFromLocationView = RMCharacterCollectionFromLocationView(viewModel: viewModel)
                    let vc: RMCharacterCollectionFromLocationViewController = RMCharacterCollectionFromLocationViewController(characterCollectionView: view)
                    self?.delegate?.stopLoading()
                    self?.delegate?.setUserInteraction(to: true)
